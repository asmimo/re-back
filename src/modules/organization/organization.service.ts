...
export class OrganizationService {
  ...
  async getOrganizations(
    ...
    if (filter) {
      ...
      if (contact != null) {
        query.where(`organization.contact ILIKE :contact`, {
          contact: `%${contact}%`,
        });
      }
      if (contact1 != null) {
        query.where(`organization.contact1 ILIKE :contact1`, {
          contact1: `%${contact1}%`,
        });
      }
      if (country != null) {
        query.where(`organization.country ILIKE :country`, {
          country: `%${country}%`,
        });
      }
      if (address != null) {
        query.where(`organization.address ILIKE :address`, {
          address: `%${address}%`,
        });
      }
      if (address1 != null) {
        query.where(`organization.address1 ILIKE :address1`, {
          address1: `%${address1}%`,
        });
      }
      if (active != null) {
