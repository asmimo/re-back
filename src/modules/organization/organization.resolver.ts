...
export class OrganizationResolver {
  ...
  async createOrganization(@Args() dto: CreateOrganizationDTO): Promise<Organization> {
    return this.licenseService.createOrganization(dto)
  }

  @Query(() => Organization)
  async getOrganization(@Args() { id }: IdDto, @Info() info: GraphQLResolveInfo): Promise<Organization> {
  ...
  @Mutation(() => Organization)
  async updateOrganization(@Args() dto: UpdateOrganizationDTO): Promise<Organization> {
